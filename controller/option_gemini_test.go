...
func TestUpdateOptionRejectsAlipayNativeKeys(t *testing.T) {
	response := httptest.NewRecorder()
	context, _ := gin.CreateTestContext(response)
	context.Request = httptest.NewRequest(http.MethodPut, "/api/option/", strings.NewReader(`{"key":"AlipayNativeEnabled","value":true}`))

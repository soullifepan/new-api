...
func TestRegistryUsesModelSpecificUsageMetadata(t *testing.T) {
	source := routingTestPluginSource(
		...
		`usageSchemaByModel: {
			"image-model": {
				images: {type: "number", unit: "count"},
				resolution: {enum: ["1k"], enumLabels: {"1k": {en: "Standard", zh: "标准"}}},
			},
			"video-model": {seconds: {type: "number", unit: "second"}},
		},
		usageExamplesByModel: {
			"image-model": [{label: "1 image", facts: {images: 1, resolution: "1k"}}],
	...
	snapshot := registry.Snapshot()
	require.Len(t, snapshot.Override, 1)
	snapshot.Override[0].UsageSchemaByModel["image-model"]["resolution"].EnumLabels["1k"]["en"] = "Changed"
	currentSchema, _ := registry.Snapshot().Override[0].UsageForModel("image-model")
	assert.Equal(t, "Standard", currentSchema["resolution"].EnumLabels["1k"]["en"])
	assert.Equal(t, "标准", currentSchema["resolution"].EnumLabels["1k"]["zh"])
}

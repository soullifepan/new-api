...
Numeric `usageSchema` fields declare a host-owned unit of `second`, `count`, `token`, `credit`, or `usd`. `usd` is a published or verified per-request USD amount, so its billing expression normally uses the field directly. Boolean fields declare `{type: "boolean"}`.

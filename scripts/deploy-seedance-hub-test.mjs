...
DO $$ BEGIN
  IF (SELECT count(*) FROM task_plugins WHERE key='seedance-hub' AND active AND version='2.0.3') <> 1 THEN
    RAISE EXCEPTION 'Expected active seedance-hub 2.0.3';
  END IF;
...
SELECT 'seedance-hub',1,'2.0.4',${quote(source)},${quote(hash)},icon,true,false,extract(epoch from now())::bigint,'Complete pricing examples for every supported resolution'
...
UPDATE task_plugins SET active=true WHERE key='seedance-hub' AND version='2.0.4';
COMMIT;
